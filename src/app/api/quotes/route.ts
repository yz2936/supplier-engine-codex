...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json(
      { quotes: quotes.sort((a, b) => b.createdAt.localeCompare(a.createdAt)) },
      { headers: { "Cache-Control": "no-store, no-cache, must-revalidate" } }
    );
  } catch {
    return NextResponse.json({ error: "Quote service temporarily unavailable. Please retry." }, { status: 503 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({ error: "Failed to save quote due to temporary service issues. Please retry." }, { status: 503 });
  }
}
